...
public class SupportPawnViewController : ViewControllerAfterLoad
{
	[SerializeField]
	...
	private void Initialize()
	{
		...
		RentalParam[1].text = string.Empty;
	}

	protected override IEnumerator LoadRoutine(ReqestResult result)
	{
		Initialize();
		yield return StartCoroutine(CharacterData.GetCharacterSupportPawnProfile(delegate(SupportPawnProfile res)
		{
			...
			else
			{
				...
				RentalParam[0].text = res.RentalAdven.ToString();
				RentalParam[1].text = res.RentalCraft.ToString();
				SingletonMonoBehaviour<ProfileManager>.Instance.SetStatusBase(res.Profile.Name, res.JobID, res.JobLv, res.ItemRank);
			}
		}, delegate
		{
			result.isError = true;
		}, CharacterId, PawnId, CacheOption.OneMinute, LoadingAnimation.Default));
	}
}
