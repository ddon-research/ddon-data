...
ImHex provides command line functionality that enables automated pattern testing and batch processing of resource files. This is particularly useful for:
The fundamental command structure for pattern evaluation is:
...
|`--pl format`
|Specifies the pattern language formatter plugin
|Yes

|`--pattern <file>`
|Path to the ImHex pattern file (`.hexpat` or `.cpp`)
|Yes

|`--input <file>` 
|Path to the binary resource file to decode
|Yes

|`--output <file>`
|Output file for JSON results (optional, defaults to stdout)
|No
|===
...
Test the rLandInfo pattern against a Season 2 resource file:

[source,bash]
----
./imhex --pl format \
  --pattern "C:\ddon-data\research\02030004\rLandInfo-pattern.hexpat" \
  --input "C:\DDO-Client\nativePC\rom\game_common\scr\land_list.lai"
----
...
[source,powershell]
----
& "C:\Program Files\ImHex\imhex.exe" --pl format `
  --pattern "D:\ddon-data\research\02030004\rLandInfo-pattern.hexpat" `
  --input "D:\DDO-Season3\nativePC\rom\base\scr\land_list.lai" `
  --output "land_info_decoded.json"
----

=== Output Interpretation

When a pattern successfully decodes a resource, ImHex outputs valid JSON to stdout (or the specified output file).
Otherwise an error is generated: `Pattern Error: 18:18 -> runtime error: Array grew past set limit of 65536`

==== Pattern Validation Script

Create a validation script to test patterns against known good files:
...
PATTERN="$1"
TEST_FILE="$2"
EXPECTED_FIELDS="$3"  # comma-separated list

echo "Validating pattern: $PATTERN"
result=$(./imhex --pl format --pattern "$PATTERN" --input "$TEST_FILE" 2>&1)
...
if [[ $? -eq 0 ]]; then
    ...
    for field in "${fields[@]}"; do
        if echo "$result" | grep -q "\"$field\""; then
            echo "✓ Found expected field: $field"
        else
...
----
./validate_pattern.sh \
    "./patterns/rLandInfo.hexpat" \
    "./test-data/land_list.lai" \
    "mLandId,mIsDispNews,mGameMode"
----