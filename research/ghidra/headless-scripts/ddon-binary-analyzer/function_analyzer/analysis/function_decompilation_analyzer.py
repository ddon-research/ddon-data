...
class FunctionDecompilationAnalyzer:
    ...
    Provides comprehensive recursive analysis of C++ functions extracted from
    Dragon's Dogma Online binaries, including call graph exploration, struct
    relationship tracking, and formatted output generation.
    """

    def __init__(self, program: Program, log_level: str = AnalyzerConfig.DEFAULT_LOG_LEVEL) -> None:
        """Initialize the analyzer with a Ghidra program and configuration.
        
        Args:
            program: Ghidra program instance containing functions to analyze
            log_level: Logging verbosity level ("DEBUG" or "INFO")
        """
        self.program = program
        self.log_level = log_level.upper()
        self.logger = LoggingConfig.setup_logger("function_decompilation_analyzer", log_level)
    ...
    def analyze_function(self, function_name: str, max_depth: int = AnalyzerConfig.DEFAULT_RECURSION_DEPTH) -> None:
        """Perform complete recursive analysis of a function and its dependencies.
        
        Args:
            function_name: Qualified function name to analyze (e.g., "rLandInfo::load")
            max_depth: Maximum recursion depth for call graph exploration (1-10)
            
        Raises:
            FunctionNotFoundException: If the specified function cannot be found
            AnalyzerException: If analysis fails due to configuration or processing errors
        """
        try:
            # Validate max_depth
    ...
    def _output_analysis_results(self, analysis_result: Dict[str, Any]) -> None:
        """Output analysis results based on configured log level.
        
        Args:
            analysis_result: Complete analysis data structure containing function
                           information, decompiled code, and dependency relationships
        """
        if self.log_level == AnalyzerConfig.INFO_LOG_LEVEL:
    ...
    def _recursive_analyze_function(self, target_function: Function, function_name: str, max_depth: int) -> Dict[
        ...
        Args:
            target_function: Ghidra Function instance to start analysis from
            function_name: Qualified name of the target function
            max_depth: Maximum recursion depth for dependency exploration
            
        Returns:
            Complete analysis result dictionary containing function hierarchy,
            decompiled code, and struct relationships across all depth levels
        """
        analyzed_functions: Set[str] = set()  # Track already analyzed functions to avoid duplicates
    ...
    def _analyze_function_level(self, functions_at_level: List[Tuple[Function, str, int]]) -> Dict[str, Any]:
        """Analyze all functions at a specific recursion level.
        
        Args:
            functions_at_level: List of tuples containing (Function, qualified_name, depth)
                              for all functions to analyze at this level
                              
        Returns:
            Level analysis result containing function analyses and decompiled code
            for all functions processed at this depth level
        """
