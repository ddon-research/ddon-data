# DWARF Symbol Explorer for Ghidra - C++ Definition Generator (PyGhidra Headless Version)
#
...
#
# PYGHIDRA HEADLESS VERSION: This version is designed for PyGhidra (pure Python 3) headless execution.
# It reads configuration from environment variables set by the run_dwarf_explorer.sh script and
# outputs results without any user interaction or GUI dependencies.
#
# CORE FUNCTIONALITY:
# - Reads symbol names from SYMBOL_NAME environment variable (set by runner script)
...
# - Outputs to stdout
#
# ENVIRONMENT VARIABLES (set by run_dwarf_explorer.sh):
# - SYMBOL_NAME: The DWARF symbol to analyze (required)
# - LOG_LEVEL: "DEBUG" or "INFO" (default: "INFO")
# - EXPLORE_DEPTH: Maximum exploration depth 1-10 (default: 5)
# - OUTPUT_FORMAT: "cpp" (only cpp format is supported)
...
from typing import Optional, Set, Dict, List, Union, Any
...
class ExplorerConfig:
    ...
    # Headless configuration - reads from environment variables set by run_dwarf_explorer.sh
    @staticmethod
    def get_symbol_name() -> Optional[str]:
        """Get symbol name from environment variable (set by run_dwarf_explorer.sh)"""
        return os.environ.get('SYMBOL_NAME', '').strip()
    ...
    @staticmethod
    def get_explore_depth() -> int:
        """Get exploration depth from environment variable"""
        try:
            depth = int(os.environ.get('EXPLORE_DEPTH', '5'))
            return max(1, min(10, depth))  # Clamp between 1-10
        except (ValueError, TypeError):
            return 5
    
    @staticmethod
    def get_output_format() -> str:
        """Get output format from environment variable (only cpp is supported)"""
        return 'cpp'
    
    @staticmethod
    def get_debug_enabled() -> bool:
        """Get debug mode from log level"""
        return ExplorerConfig.get_log_level() == 'DEBUG'
    
    @staticmethod
    def should_exit_on_error() -> bool:
        """Check if script should exit on error"""
        return os.environ.get('EXIT_ON_ERROR', 'true').lower() in ('true', '1', 'yes')
...
class HeadlessOutputHandler:
    """Handler for managing output in headless PyGhidra mode"""
...
class DwarfSymbolExplorerHeadless:
    """
    Headless version of DWARF symbol explorer.
    
    This class provides the same functionality as the interactive version
    but removes all GUI dependencies and user prompts, reading configuration
    from environment variables instead.
    """
    
    def __init__(self, program: Program):
        """Initialize the headless explorer with a Ghidra program"""
        self.program = program
        self.output_handler = HeadlessOutputHandler()
    ...
    def find_and_explore_symbol(self, symbol_name: str) -> Optional[DataType]:
        ...
        self.logger.debug(f"\n{'='*60}")
        self.logger.debug("DWARF SYMBOL EXPLORER HEADLESS STARTING")
    ...
    def run_complete_analysis(self, symbol_name: str) -> bool:
        """Run complete analysis of a symbol, returns True on success"""
        try:
            # Find the symbol
            ...
            if ExplorerConfig.get_debug_enabled():
                import traceback
    ...
    def _print_completion_summary(self, symbol_name: str) -> None:
        ...
        self.logger.debug(f"Log Level: {ExplorerConfig.get_log_level()}")
        self.logger.debug(f"{'='*60}")


def main():
    """Main function to run the PyGhidra headless DWARF symbol explorer"""
    if currentProgram is None:
        print("ERROR: No program is currently loaded")
        sys.exit(1) if ExplorerConfig.should_exit_on_error() else None
        return
    
    # Get symbol name from environment variable (set by run_dwarf_explorer.sh)
    symbol_name = ExplorerConfig.get_symbol_name()
    if not symbol_name:
        print("ERROR: SYMBOL_NAME environment variable is not set")
        print("This script should be run via the run_dwarf_explorer.sh script:")
        print("  ./run_dwarf_explorer.sh \"rLandInfo\" \"DDOORBIS.elf\" \"INFO\" 5 \"cpp\"")
        sys.exit(1) if ExplorerConfig.should_exit_on_error() else None
        return
    
    # Create and run the headless explorer
    explorer = DwarfSymbolExplorerHeadless(currentProgram)
    success = explorer.run_complete_analysis(symbol_name)
